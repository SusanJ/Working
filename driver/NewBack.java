...
import org.dotlessbraille.transtables.PartWordUse;
...
public void enterLargeSign(uebMathParser.LargeSignContext ctx) {
  ...
  String cap = null;
   //Check info as to whether parent is a simple word and
   //if so, whether it has a leading cap letter idicator
   //FIX?? WHAT ABOUT UC
  if (ctx.getParent().getText() != null){
   if (ctx.getParent().getText().startsWith( ",")) cap="," ; 
  }
...
// Lower sign contraction, translation depends on position
@Override 
public void enterLowsigncon(uebMathParser.LowsignconContext ctx) { 
//PartWordUse BEG_WORD,
//MID_WORD,
//en and in anywhere?
 if (showEnter) System.out.println( "enterLowsigncon" );
 PartWordUse use = PartWordUse.BEG_WORD;
 int pruleIndex = ctx.getParent().getRuleIndex();
 String parentText = ctx.getParent().getText();
 String pruleName = ruleNames[pruleIndex];
 System.out.println( "enterText--pruleName: "+pruleName );
 if (pruleName.equals( "wordMiddle" )) use = PartWordUse.MID_WORD;
 TransWrapper.wordCase opt = TransWrapper.wordCase.LC;
 String brl = ctx.getText();
 if (debug) System.out.println( "low con brl: "+brl );
 String ink = TransWrapper.btLOW( brl, use, opt );
 if (debug) System.out.println( "low con ink: "+ink );
 setInk( ctx, ink );
}
@Override 
public void exitLowsigncon(uebMathParser.LowsignconContext ctx) {
if (showExit) System.out.println( "exitLowsigncon" );
 }
...
@Override
public void enterShortForms(uebMathParser.ShortFormsContext ctx) { 
 String ink = 
TransWrapper.btShortform( ctx.getText(), TransWrapper.wordCase.LC );
 setInk( ctx, ink );
}
@Override 
...
public void enterWord(uebMathParser.WordContext ctx) { 
  int ruleIndex = ctx.getRuleIndex();
  if (debug){
   System.out.println( "enterWord--ruleName: "+ruleNames[ruleIndex] );
   int pruleIndex = ctx.getParent().getRuleIndex();
