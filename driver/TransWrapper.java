...
//   Adapter for using classes from the transtables package 
// for backtranslation of words
...
System.out.println( "TW lrg cap: "+cap);
  String ink = null;
  if (cap == null){
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// 10.5 Lower Wordsigns (In special standing alone context.)
 
public static String btLOW( String brl ){
 if (brl.length() == 1) return btLOW( brl, wordCase.LC );
 return btLOW( brl.substring(1), wordCase.TC );
}
public static String btLOW( String brl, wordCase opt ){
 switch (opt){ 
  case LC:
   return WordOrPartWord.lowBackLC( brl );
  case TC:
   return WordOrPartWord.lowBackTC( brl );
  case UC:
   return WordOrPartWord.lowBackUC( brl );
 }
 return null;
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// 10.6 Lower GroupSigns 
// Note that the back translators all backtranslate the
// ANYWHERE contractions appropriately, e.g.  "en" and "in"

  public static String btLOW( String brl, PartWordUse use,
     wordCase opt){
  System.out.println("Wrapper btLOW: "+ brl) ;
  switch (use){
   case BEG_WORD:
    switch (opt){
     case LC:
...
//  Section 10.9 Shortforms
  ...
  public static String btShortform( String brl, wordCase opt){
   switch (opt){ 
    case LC:
     return WordOrPartWord.sfBackLC( brl );
    case TC:
     return WordOrPartWord.sfBackTC( brl );
    case UC:
     return WordOrPartWord.sfBackUC( brl );
   }
   return null;
  }
...
// Words with one or more contractions and possibly letters
 ...
 public static String btApp1Word( String capLetter, String rest ){
  StringBuilder buf = new StringBuilder( capLetter.substring( 1 ) );
  buf.append( rest );
  String ink = App1Word.getApp14Brl( buf.toString() );
  if (ink == null) return null;
  return toTC( ink );
 }
