...
public void enterStrong_gs(uebMathParser.Strong_gsContext ctx) {
 ...
 String ink = backStrongGS( brl, false );
 if (debug){
...
@Override public void enterIlc(uebMathParser.IlcContext ctx) { 
  ...
  String brl = ctx.getChild(0).getText();
  System.out.println( "enterIlc brl: "+brl );
  String ink = null;
  if (cap == null){
   ink = backILClc( brl );
  } else {
...
static String backILCcc( String brl ){
...
public void enterAlphaSeq(uebMathParser.AlphaSeqContext ctx) { 
 if (showEnter) System.out.println( "   Enter AlphaSeq" );
  ...
  String ink = null; 
  if (debug){
   System.out.println( "          enterAlpha--cap: "+cap );
...
public void enterSimpleWord(uebMathParser.SimpleWordContext ctx) { 
 ...
 //Check for leadding caps letter indicator (TC word)
 String cap = ctx.getChild(0).getText(); 
 if (cap.equals( "," )) setInfo( ctx, cap );
}
@Override 
...
public void enterWord(uebMathParser.WordContext ctx) { 
 ...
 //Check for caps letter indicator
 String cap = ctx.getChild(0).getText(); 
 System.out.println( "EnterWord cap: "+ cap );
 if (cap.equals( "," )) setInfo( ctx, cap );
}
...
public void enterSingleLetter(uebMathParser.SingleLetterContext ctx) {
 ...
 String ink = backLetterAsWord( ctx.getText() );
 if (debug){
...
public void exitSingleLetter(uebMathParser.SingleLetterContext ctx) {
   ...
   boolean inExpr = true;
   if (expr2Count <= 0) {
    System.out.println( "exitSingleLetter -- no expt");
    inExpr = false;
   }
...
String ink = conCat( ctx, "separator", true, 0 );
 setInk( ctx, ink );
 }
